...
class Helper 
{
    ...
    public static function generatePermohonanKode()
    {
        return 'PM' . date('ymd') . mt_rand(100000, 999999);
    }
    ...
    public static function statusLayanan($value) { 
        switch ($value) {
            case 0:
                return '<span class="badge bg-danger bg-gradient badge-sm mx-1">Gangguan</span>';
                break;

            case 1:
                return '<span class="badge bg-success bg-gradient badge-sm mx-1">Normal</span>';
                break;
    
            default:
                return '<span class="badge bg-dark bg-gradient badge-sm mx-1">unknown</span>';
                break;
        }
    }
    ...
    public static function tanggalIndoWithJam($tanggal, $cetak_hari = 0) {
        ...
        if ($cetak_hari == 1) {
            $num = date('N', strtotime($tanggal));
            $split    = explode('-', $tanggal);
            $tgl_indo = $hari[$num] . ', ' . $split[2] . ' ' . $bulan[ (int)$split[1] ] . ' ' . substr($split[0], 2, 2) . ' ' . $jam;
        } else {
            $split    = explode('-', $tanggal);
